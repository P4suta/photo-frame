...
//! Centralising it behind a single named function lets the geometry code
//! read in *domain* terms (`round_to_u32(x)` vs `x as u32`), and keeps the
//! one place where a checked-cast lint is silenced explicit and audited.

/// Round `v` to the nearest `u32`.
///
/// Non-finite or strictly-negative inputs return `0`; values larger than
/// [`u32::MAX`] saturate at the top.
#[allow(
